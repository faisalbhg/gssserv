...
@endpush
 ...
 <main class="main-content position-relative  border-radius-lg">
    ...
    <div class="container-fluid py-2">
        ...
        <div class="row">
            @if($newVehicleSearch)
            <div class="col-lg-2 col-sm-3 mt-lg-0 mt-4">
                <div class="card bg-gradient-primary h-100 cursor-pointer" wire:click="newVehicleOpen">
                    <div class="card-body p-2">
                        ...
                        <a href="javascript:;" class="text-dark">
                            <div class="icon icon-shape shadow text-center border-radius-md">
                                <i class="fa-solid fa-car fa-xl opacity-10" aria-hidden="true"></i>
                            </div>
                            <h5 class="text-secondary text-white"><i class="fa fa-plus text-white mb-3" aria-hidden="true"></i> New Vehicle </h5>
                        </a>
                    </div>
                </div>
            </div>
            ...
            @if($showDashboard)
                @forelse($pendingCustomersCart as $pendingvehicle)
                    @if(!in_array($pendingvehicle->customer_id, $arrayCustomersPendingJobs))
                        <div class="col-lg-2 col-sm-3 mt-lg-0 mt-4">
                            <div class="card h-100 cursor-pointer" wire:click="selectPendingVehicle('{{$pendingvehicle}}')">
                                <div class="card-body p-2">
                                    <div class="d-flex mb-4">
                                        <p class="mb-0">{{$pendingvehicle->vehicleInfo['plate_number_final']}}</p>
                                    </div>
                                    <img src="{{url('storage/'.$pendingvehicle->vehicleInfo['vehicle_image'])}}" class="w-100 rounded-3 shadow-lg">
                                    <p class="mt-4 font-weight-bold mb-0">{{$pendingvehicle->vehicleInfo['make']}} - {{$pendingvehicle->vehicleInfo['model']}}</p>
                                    <span class="text-xs">{{ \Carbon\Carbon::parse($pendingvehicle->created_at)->diffForHumans() }} </span>
                                </div>
                            </div>
                        </div>
                        <?php array_push($arrayCustomersPendingJobs, $pendingvehicle->customer_id);?>
                    @endif
                @empty
                @endforelse
            @endif
        ...
        <div class="row mt-4">
            <div class="col-lg-6 col-7">
                <h6>Pending Payment Customers</h6>
            </div>
            <div class="col-lg-6 col-5 my-auto text-end">
        ...
        <div class="row mt-2">
            @forelse($pendingjobs as $pendingjob)
                <div class="col-lg-4 col-md-6 col-12 my-2">
                    <div class="card h-100 bg-gradient-secondary cursor-pointer" wire:click="pendingPaymentClick('{{$pendingjob}}')">
                        <div class="card-body p-3">
                            <div class="row">
                                <div class="col-8">
                                    <div class="numbers">
                                        <p class="mt-2 mb-0"><span class="badge ">{{$pendingjob->customerVehicle['make']}}-{{$pendingjob->customerVehicle['model']}}
                                        </span></p>
                                        <p><span class="badge ">{{$pendingjob->customerVehicle['plate_number_final']}}</span></p>
                                ...
                                <div class="col-4 text-end">
                                    <div class="icon icon-shape bg-white shadow text-center border-radius-md w-100">
                                        <img src="{{url('storage/'.$pendingjob->customerVehicle['vehicle_image'])}}" class="w-100 rounded-3 shadow-lg">
                                    </div>
                                </div>
                            </div>
                        </div>
